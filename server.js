...
const server=http.createServer(async(req,res)=>{ try{ const url=new URL(req.url,`http://${req.headers.host}`);
  ...
  if(url.pathname==='/api/operators') { const db=loadDb(); return send(res,200,{operators:operatorList(db)}); }
