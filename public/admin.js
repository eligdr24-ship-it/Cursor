...
function bindPreviewCells(){
  document.querySelectorAll('.preview-cell').forEach(el=>{
    ...
    el.onclick=e=>{if(window.matchMedia('(max-width: 760px)').matches && el.tagName!=='A'){e.preventDefault();e.stopPropagation();openFullCell(el.dataset.full||el.textContent||'')}};
  });
}
...
function renderHistory(history){$('historyRows').innerHTML=history.length?history.map(h=>`<tr data-history-id="${esc(h.id||'')}"><td class="select-cell" data-label="Select"><input type="checkbox" class="history-row-check" data-history-id="${esc(h.id||'')}"></td><td class="nowrap" data-label="Date & Time">${esc(dateTimeCell(h.at,h.day))}</td><td data-label="Operator">${esc(h.userName||h.userId||'User')}</td><td class="business-tab-cell" data-label="Business Tab">${compactText(h.source||'Links','text')}</td><td class="break" data-label="Original Link">${compactLink(h.link)}</td><td class="break" data-label="Submitted Link">${submittedCell(h)}</td><td data-label="Text">${compactText(h.text,'text')}</td><td data-label="Admin Notes">${noteCell(h)}</td><td data-label="Status / Actions">${actionCell(h)}</td></tr>`).join(''):'<tr class="empty-row"><td colspan="9">No submissions found for this date range.</td></tr>';setText('resultCount',`Showing ${history.length} result${history.length===1?'':'s'}`);bindHistoryInputs();bindPreviewCells();bindHistorySelection();setTimeout(()=>window.initAdminHistoryColumnResize&&window.initAdminHistoryColumnResize(),0)}
...
async function loadTrash(){const el=$('trashRows'); if(!el)return; try{const j=await api('/api/admin/history/trash'); const rows=j.trash||[]; el.innerHTML=rows.length?rows.map(h=>`<tr><td data-label="Deleted / Date">${esc(dateTimeCell(h.deletedAt||h.at,h.day))}</td><td data-label="Operator">${esc(h.userName||h.userId||'User')}</td><td data-label="Business Tab">${compactText(h.source||'Links')}</td><td data-label="Original Link">${compactLink(h.link)}</td><td data-label="Submitted Link">${compactLink(h.submittedLink||'')}</td><td data-label="Text">${compactText(h.text||'')}</td><td data-label="Action"><button class="btn blue tinyBtn restore-history-btn" data-history-id="${esc(h.id)}" type="button">Restore</button></td></tr>`).join(''):'<tr class="empty-row"><td colspan="7">Trash is empty.</td></tr>'; document.querySelectorAll('.restore-history-btn').forEach(b=>b.onclick=()=>restoreHistoryRow(b.dataset.historyId)); bindPreviewCells();}catch(e){el.innerHTML='<tr class="empty-row"><td colspan="7">Could not load trash.</td></tr>';}}
