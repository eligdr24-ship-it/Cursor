let LAST=null;const $=id=>document.getElementById(id);const USERS={operator1:{id:'operator1',name:'Operator 1'},operator2:{id:'operator2',name:'Operator 2'},operator3:{id:'operator3',name:'Operator 3'},operator4:{id:'operator4',name:'Operator 4'}};function uid(){const p=location.pathname.split('/').filter(Boolean);const maybe=String(p[1]||p[0]||'').toLowerCase().replace(/[^a-z0-9_-]/g,'');return /^operator\d+$/.test(maybe)?maybe:'operator1'}const USER_ID=uid();let USER=USERS[USER_ID]||{id:USER_ID,name:`Operator ${USER_ID.replace('operator','')||''}`.trim()};const PASS_KEY=`grr_operator_pass_${USER_ID}`;function pass(){return localStorage.getItem(PASS_KEY)||''}
function setDisplayName(name){if(!name)return;USER.name=name;document.title=`${USER.name} Analytics`;const t=$('userNameTitle');if(t)t.textContent=USER.name;}
...
function compactLink(url){const val=String(url||'').trim();if(!val)return '<span class="muted-inline compact-cell">No link</span>';const safe=esc(val),href=esc(hrefFor(val));return `<a class="compact-cell preview-cell compact-link" href="${href}" target="_blank" rel="noopener" title="${safe}" data-full="${safe}">${safe}</a>`}
...
function bindPreviewCells(){document.querySelectorAll('.preview-cell').forEach(el=>{el.ondblclick=e=>{e.preventDefault();e.stopPropagation();openFullCell(el.dataset.full||el.textContent||'')};el.onclick=e=>{if(window.matchMedia('(max-width: 760px)').matches&&el.tagName!=='A'){e.preventDefault();e.stopPropagation();openFullCell(el.dataset.full||el.textContent||'')}}})}
...
function submittedCell(h){const val=String(h.submittedLink||'');const safe=esc(val),href=esc(hrefFor(val));const id=esc(h.id||'');if(val.trim())return `<span class="history-lock-field is-locked"><a class="compact-cell preview-cell compact-link" href="${href}" target="_blank" rel="noopener" title="${safe}" data-full="${safe}">${safe}</a><input class="history-edit submitted-edit" data-history-id="${id}" value="${safe}" /></span>`;return `<span class="history-lock-field is-locked"><span class="muted-inline compact-cell">Paste submitted link...</span><input class="history-edit submitted-edit" data-history-id="${id}" value="" placeholder="Paste submitted link..." /></span>`}
...
function renderHistory(hist){$('historyRows').innerHTML=hist.length?hist.map(h=>`<tr data-history-id="${esc(h.id||'')}"><td data-label="Time">${esc(localTime(h.at))}</td><td data-label="Day">${esc(h.day)}</td><td data-label="Business Tab">${compactText(h.source||'Links')}</td><td class="break" data-label="Original Link">${compactLink(h.link)}</td><td class="break" data-label="Submitted Link">${submittedCell(h)}</td><td data-label="Text">${compactText(h.text)}</td><td data-label="Status / Actions">${actionCell(h)}</td></tr>`).join(''):'<tr class="empty-row"><td colspan="7">No submissions for this date range.</td></tr>';setText('resultCount',`Showing ${hist.length} result${hist.length===1?'':'s'}`);bindHistoryInputs();bindPreviewCells()}
...
async function loadReport(){const j=await api(`/api/report?${dateParams()}&user=${USER_ID}`);LAST=j;if(j.status&&j.status.user)setDisplayName(j.status.user.name);fill(j.status);renderWeeklyGoalBox(j.weekly);renderHistory(j.report.history);drawChart(j.report.daily);renderActivity(j.report.history)}
